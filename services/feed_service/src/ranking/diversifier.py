...
class FeedDiversifier:
    ...
    def _enforce_author_diversity(self, posts: list[ScoredPost]) -> list[ScoredPost]:
        ...
        for post in posts:
            ...
            author_count = sum(1 for p in window if p.author_id == post.author_id)
        ...
        # Re-insert deferred posts respecting the same window constraint
        ...
        for _ in range(max_passes):
            ...
            for post in remaining:
                ...
                if author_count < self.MAX_AUTHOR_PER_WINDOW:
                    ...
                    still_deferred.append(post)
            if len(still_deferred) == len(remaining):
                result.extend(still_deferred)
                break
    ...
    def _enforce_media_diversity(self, posts: list[ScoredPost]) -> list[ScoredPost]:
